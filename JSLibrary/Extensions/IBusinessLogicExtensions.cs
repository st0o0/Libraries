...
namespace JSLibrary.Extensions
{
    public static class IBusinessLogicExtensions
    {
        ...
        public static async Task UpdateManyAsync<ModelType, DBContextType>(this IBusinessLogicBase<ModelType, DBContextType> businessLogic, IEnumerable<ModelType> items, CancellationToken cancellationToken = default) where DBContextType : DbContext where ModelType : class, new()
        {
            await ParallelTask.TaskManyAsync(items, async x => await businessLogic.UpdateAsync(x, cancellationToken), 1, cancellationToken);
        }
